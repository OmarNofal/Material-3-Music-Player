...
class LyricsRepository @Inject constructor(
    ...
    suspend fun getLyrics(
        ...
        kotlin.run {
            val lyrics = tags.getFirstField(FieldKey.LYRICS)?.toString() ?: ""
            val syncedLyrics = SynchronizedLyrics.fromString(lyrics)
            if (syncedLyrics != null)
                return@withContext LyricsResult.FoundSyncedLyrics(syncedLyrics)
            else if (lyrics.isNotBlank())
                return@withContext LyricsResult.FoundPlainLyrics(PlainLyrics(lyrics))
        }
        ...
        kotlin.run {
            ...
            if (lyricsEntity != null) {
                ...
                if (lyricsEntity.syncedLyrics.isNotBlank()) {
                    ...
                    if (synced != null) {
                        return@withContext LyricsResult.FoundSyncedLyrics(synced)
                    } else {
                        return@withContext LyricsResult.FoundPlainLyrics(
                            PlainLyrics(lyricsEntity.plainLyrics)
                        )
                    }
                }
        ...
        return@withContext try {
            ...
            lyricsDao.saveSongLyrics(
                LyricsEntity(0, title, album, artist, lyricsNetwork.plainLyrics, lyricsNetwork.syncedLyrics)
            )
            if (syncedLyrics != null) LyricsResult.FoundSyncedLyrics(syncedLyrics)
            else LyricsResult.FoundPlainLyrics(PlainLyrics(lyricsNetwork.plainLyrics))
